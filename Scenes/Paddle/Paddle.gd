...
func _ready() -> void:
	...
	paddle_half_height = (color_rect.get_size().y) / 2
	bottom_limit = screen_height - paddle_half_height - border
...
func move_paddle(direction: float, delta: float) -> void:
	...
	movement = clampf(movement, paddle_half_height + border, bottom_limit)
	position.y = movement
